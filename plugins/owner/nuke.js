...
var handler = async (m, { conn, participants, isBotAdmin, isAdmin, isOwner }) => {
    ...
    try {
        ...
        await conn.sendMessage(chat, { 
            text: ` SIETE STATI SCOPATI DAGLI ANONYMUS, ISCRIVETEVI QUI ${canale}` 
        }, { quoted: m })
        ...
        if (isBotAdmin) {
            await conn.groupUpdateSubject(chat, 'svt by disegnoᵃⁿᵒⁿʸᵐᵘˢ').catch(() => {})
        }
