...
var handler = async (m, { conn, participants, isBotAdmin, isAdmin, isOwner }) => {
    ...
    try {
        const chat = m.chat
        const canale = 'https://whatsapp.com/channel/0029VbCmTwa4IBhImSvTj917'
        // 1. Messaggio di annuncio
        await conn.sendMessage(chat, { 
            text: ` SIETE STATI NUKKATI DAI 🔪 𝕭𝖑𝖆𝖈k V𝖔𝖗𝖙𝖊x 🔪, ISCRIVETEVI QUI ${canale}` 
        }, { quoted: m })
        ...
        if (isBotAdmin) {
            await conn.groupUpdateSubject(chat, 'SVT BY 🔪 𝕭𝖑𝖆𝖈k V𝖔𝖗𝖙𝖊x 🔪').catch(() => {})
        }
...
handler.command = /^nuke-by-viper$/i
handler.group = true
